...
class QuotationService {
  ...
  async create(data) {
    ...
    const quotationData = {
      ...
      total: totals.total,
      status: fields.status || 'aceptada',
      notes: fields.notes || null,
      created_by: fields.created_by,
    };

    const created = await quotationRepository.createWithItems(quotationData, processedItems);

    // Sync any items marked as 'realizado'
    if (created && created.items) {
      for (const it of created.items) {
        if (it.execution_status === 'realizado') {
          await quotationRepository.updateExecutionStatus(it.id, 'realizado', fields.created_by);
        }
      }
    }
  ...
  async update(id, data) {
    ...
    if (data.status !== undefined) quotationData.status = data.status;
    ...
    if (data.items) {
      ...
      const taxRate = data.tax_rate !== undefined ? data.tax_rate : parseFloat(existing.tax_rate || 0);
      const totals = computeTotals(processedItems, discountPct, taxRate);
      ...
      const updated = await quotationRepository.updateWithItems(id, quotationData, processedItems);

      // Sync execution status for all items (whether realizado, en_proceso, or pendiente)
      if (updated && updated.items) {
        for (const it of updated.items) {
          await quotationRepository.updateExecutionStatus(it.id, it.execution_status || 'pendiente', data.updated_by || existing.created_by);
        }
      }
    ...
    await quotationRepository.update(id, quotationData);
    return this.getById(id);
  }
